...
#define INNER_HEAP_SIZE (512 * 1024)
...
static void video_thread(void* arg) {
    ...
    while (g_running) {
        if (captureReadVideo(&frame)) {
            if (g_capture_enabled) (void)enqueue_video(&frame, g_generation);
        } else {
...
static void audio_thread(void* arg) {
    ...
    while (g_running) {
        if (captureReadAudio(&frame)) {
            if (g_capture_enabled) (void)enqueue_audio(&frame, g_generation);
        } else {
            svcSleepThread(2 * 1000 * 1000LL);
        }
    }
